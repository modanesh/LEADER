...
#include <chrono>
#include <ros/ros.h>
#include <car_hyp_despot/ImportanceDistService.h>
...
// code by Mohamad
double HiddenStateBelief::IS_Sample(int& intention_id, int& mode_id, std::vector<double> importance_dist) {
    this->Normalize();

    double r = Random::RANDOM.NextDouble();
    int mode = AGENT_ATT;
    auto& orig_sampling_dist = probs_[mode];
    importance_dist.resize(orig_sampling_dist.size());
    for (int i = 0; i < probs_.size(); i++)
    {
        for (int j = 0; j < probs_[i].size(); j++)
        {
            cout << probs_[i][j];
        }
    }
    double intention_importance_w;

    // calculating the importance weights for specific intentions of individual agents
    std::vector<double> importance_weights;
    importance_weights.resize(orig_sampling_dist.size());
    std::transform(orig_sampling_dist.begin(), orig_sampling_dist.end(), importance_dist.begin(), importance_weights.begin(),
                   std::divides<double>());

    // sampling intentions from the importance distribution
    double total_prob = std::accumulate(importance_dist.begin(), importance_dist.end(), 0.0);
    for (int intention = 0; intention < orig_sampling_dist.size(); intention++) {
        r -= importance_dist[intention] / total_prob;
        if (r <= 0.001) {
            intention_id = intention;
            mode_id = mode;
            intention_importance_w = importance_weights[intention_id];
            break;
        }
    }

//    if (r > 0.001)
//        ERR("Sampling belief failed, r:" + to_string(r));

    return intention_importance_w;
}
...
// code by Mohamad
double AgentBelief::IS_Sample(int& intention, int& mode, std::vector<double> importance_dist) const {
    return belief_->IS_Sample(intention, mode, importance_dist);
}
...
std::vector<State*> CrowdBelief::Sample(int num) const {
	...
	for (int i = 0; i < num; i++) {
		particles_[i] = model_->Allocate(i, unit);
	}
	...
	for(auto* particle: particles_) {
        ...
        double particle_importance_w = 1.0;
		int agent_id = 0;
		for (std::map<double,AgentBelief*>::const_iterator it=sorted_belief_.begin(); it!=sorted_belief_.end(); ++it) {
            if (agent_id < state->num) {
                ...
                ros::NodeHandle n;
                ros::ServiceClient client = n.serviceClient<car_hyp_despot::ImportanceDistService>("importance_weight_calculation");
                car_hyp_despot::ImportanceDistService srv;
                vector<double> context;
                context.insert(context.end(), {state->agents[agent_id].pos.x, state->agents[agent_id].pos.y,
                                               state->agents[agent_id].mode, state->agents[agent_id].intention,
                                               state->agents[agent_id].pos_along_path, state->agents[agent_id].cross_dir,
                                               state->agents[agent_id].id, state->agents[agent_id].type,
                                               state->agents[agent_id].speed, state->agents[agent_id].vel.x,
                                               state->agents[agent_id].vel.y, state->agents[agent_id].heading_dir,
                                               state->agents[agent_id].bb_extent_x, state->agents[agent_id].bb_extent_y});
                srv.request.context = context;
                vector<double> to_be_sent_belief = b->belief_->Extract();
                srv.request.belief = to_be_sent_belief;
                vector<double> importance_dist;
                if (client.call(srv)) {
                    importance_dist = srv.response.weights;
                    double importance_w = b->IS_Sample(state->agents[agent_id].intention, state->agents[agent_id].mode, importance_dist);
                    particle_importance_w *= importance_w;
                } else {
				...
				world_model_.ValidateIntention(state->agents[agent_id].id, state->agents[agent_id].intention, __FUNCTION__, __LINE__);
                agent_id++;
			} else
				break;
		}
		particle->weight = unit * particle_importance_w;
	}
// TODO: should this part be commented? the reason to ask this question is that with importance weights, the sum of the
//  weights don't necessarily sum to one
